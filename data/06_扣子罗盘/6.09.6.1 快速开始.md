原文地址：https://docs.coze.cn/cozeloop/quickstart-java-sdk
...
使用扣子罗盘 Java SDK 前需完成鉴权配置。详细说明请参考 [SDK 鉴权](https://docs.coze.cn/cozeloop/authentication-for-sdk)。
...
export COZELOOP\_WORKSPACE\_ID="your\_workspace\_id"

export COZELOOP\_API\_TOKEN="your\_pat\_token"

- **COZELOOP\_WORKSPACE\_ID**: 目标工作空间 ID。

- **COZELOOP\_API\_TOKEN**: 个人访问令牌。
...
export COZELOOP\_JWT\_OAUTH\_CLIENT\_ID="your\_client\_id"

export COZELOOP\_JWT\_OAUTH\_PRIVATE\_KEY="your\_private\_key"

export COZELOOP\_JWT\_OAUTH\_PUBLIC\_KEY\_ID="your\_public\_key\_id"
...
# workspace-id: your\_workspace\_id

# auth:

# token: your\_pat\_token # pat token 和 jwt 二选一即可
...
try (CozeLoopSpan rootSpan = client.startSpan("process\_user\_request", "custom")) {
...
try (CozeLoopSpan rootSpan = client.startSpan("main\_process", "custom")) {

// 创建子 Span，自动关联父级 Span

try (CozeLoopSpan childSpan = client.startSpan("database\_query", "db")) {
...
try (CozeLoopSpan modelSpan = client.startSpan("llm\_call\_openai", "model")) {
...
直接调用 SDK 执行 Prompt，支持流式与非流式。流式调用的详情请参阅 [调用 Prompt](https://docs.coze.cn/cozeloop/ptaas-for-java-sdk)。
...
| Prompt   | <https://github.com/coze-dev/cozeloop-java/tree/main/examples/src/main/java/prompt/prompt_hub> | Prompt 拉取与管理 |

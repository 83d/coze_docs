原文地址：https://docs.coze.cn/cozeloop/best-practices-java-sdk
...
1. **创建 Prompt**：在工作空间中新建 Prompt，编写提示词内容并定义变量（如 {{query}}）。

2. **获取 Prompt Key**：进入 Prompt 详情页，复制 **Prompt Key**，这是 SDK 调用的唯一凭证。

3. **发布版本**：完成调试后，务必**提交版本**。SDK 仅能拉取已提交版本的 Prompt。建议为版本设置 label（如 production）以便管理。
...
// SDK 会自动从环境变量 COZELOOP\_WORKSPACE\_ID 和 COZELOOP\_API\_TOKEN 读取配置。
...
try (CozeLoopSpan rootSpan = client.startSpan("travel\_plan\_generation", "custom")) {

rootSpan.setTag("user\_id", "user\_67890");

rootSpan.setTag("request\_source", "mobile\_app");
...
executePrompt(client, "your\_prompt\_key", "你好");
...
try (CozeLoopSpan modelSpan = client.startSpan("llm\_call\_doubao", "model")) {
...
if (promptKey == null || promptKey.equals("your\_prompt\_key")) {

System.out.println("\n提示：请替换 `your\_prompt\_key` 为你在平台创建的实际 Prompt Key 来测试执行功能。");

return;

}

try (CozeLoopSpan executeSpan = client.startSpan("execute\_prompt\_example", "custom")) {
...
executeSpan.setTag("prompt\_key", promptKey);
...
executeSpan.setTag("total\_tokens", result.getUsage().getTotalTokens());

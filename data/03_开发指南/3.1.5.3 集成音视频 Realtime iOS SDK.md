原文地址：https://docs.coze.cn/dev_how_to_guides/realtime_iOS
...
| 准备开发环境 | - macOS 电脑，且可以正常访问互联网。  - Xcode 14.1 或以上版本。本示例采用16.2 版本。  - Apple 开发者账号，开发阶段可以使用个人免费账号。  - iOS 11.0 或以上版本真机设备，且可以正常访问互联网。本示例采用 iOS 18.1。  - 安装 CocoaPods。 |
...
- Swift
  ...
  1. 在项目根目录下，复制coze-swift/Config/APIConfig.swift.template文件为。

  2. 编辑APIConfig.swift配置文件，配置 accessToken 和 botId。
  ...
  static let accessToken = "pat\_sWaR9V4Yr8\*\*\*" // 替换为从 https://www.coze.cn/open/oauth/pats 获取到的token
  ...
  - **voiceId**：音色 ID。可选。你可以调用[查看音色列表](https://docs.coze.cn/developer_guides/list_voices) API 查看当前可用的音色列表，也可以使用[系统音色列表](https://docs.coze.cn/dev_how_to_guides/sys_voice)。


4. 编译并运行项目。

1. 使用 Xcode 打开coze-swift.xcworkspace或 coze-objc.xcworkspace。
...
- -
    ...
    <img src="https://p9-arcosite.byteimg.com/tos-cn-i-goo7wpa0wc/e93461f33405432d99de1d06e5b573d4~tplv-goo7wpa0wc-quality:q75.awebp" width="235">


  [](https://p9-arcosite.byteimg.com/obj/tos-cn-i-goo7wpa0wc/38f98a104b21455cb60a9b127e5cc9b1)
...
-
  ...
  <img src="https://p9-arcosite.byteimg.com/tos-cn-i-goo7wpa0wc/8d3f9fea28834bc7a5235024124d65ac~tplv-goo7wpa0wc-quality:q75.awebp" width="444">
...
<img src="https://p9-arcosite.byteimg.com/tos-cn-i-goo7wpa0wc/842abb18462a44f494b219bb3b543327~tplv-goo7wpa0wc-quality:q75.awebp" width="1960">
...
<img src="https://p9-arcosite.byteimg.com/tos-cn-i-goo7wpa0wc/df8bc4065f2946b6afe63583c9909d99~tplv-goo7wpa0wc-quality:q75.awebp" width="850">
...
<string>$(PRODUCT\_MODULE\_NAME).SceneDelegate</string>
...
-
  ...
  <img src="https://p9-arcosite.byteimg.com/tos-cn-i-goo7wpa0wc/43b31dad4b4e4a6c8bae47110f06ceb3~tplv-goo7wpa0wc-quality:q75.awebp" width="800">
...
- > [!NOTE]
  ...
  >
  > 如果编译过程中提示错误，请参见[常见问题](https://docs.coze.cn/dev_how_to_guides/realtime_iOS)。
...
调用扣子编程的[创建房间](https://docs.coze.cn/developer_guides/create_room)接口创建房间，通过 NetworkService 获取房间信息。
...
roomInfo.app\_id, delegate: self, parameters: [:])
...
self.rtcRoom = self.rtcVideo?.createRTCRoom(roomInfo.room\_id)
...
1. 依次调用 leaveRoom 和destroy 接口离开并销毁房间。

2. 调用destroyRTCVideo 接口释放引擎资源，避免内存泄漏。
...
**if** messageData.event\_type == "conversation.message.delta"

|| messageData.event\_type == "conversation.message.completed"

{

**let** content = messageData.data?.content ?? ""

**self**.addMessage(content, eventType: (messageData.event\_type)!)
...
<img src="https://p9-arcosite.byteimg.com/tos-cn-i-goo7wpa0wc/83e4d9e67b1243aab894fd22412f4a92~tplv-goo7wpa0wc-quality:q75.awebp" width="800">

### 编译时提示 No such module 'VolcEngineRTC'

请使用 iOS 真机进行测试，不支持使用模拟器运行。

<img src="https://p9-arcosite.byteimg.com/tos-cn-i-goo7wpa0wc/c9d586da690b42ac809907d185a98441~tplv-goo7wpa0wc-quality:q75.awebp" width="800">

### 提示 doesn't match RTCDemo-Swift.app's iOS deployment target

<img src="https://p9-arcosite.byteimg.com/tos-cn-i-goo7wpa0wc/00d46886cb1a4409a44035b00b22a99b~tplv-goo7wpa0wc-quality:q75.awebp" width="600">
...
<img src="https://p9-arcosite.byteimg.com/tos-cn-i-goo7wpa0wc/ecd24e44926447cdbf1ddaeaa3a63fa3~tplv-goo7wpa0wc-quality:q75.awebp" width="800">
